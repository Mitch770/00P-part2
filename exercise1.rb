...
Add another class variable called @@accounts that starts as an empty array. This will eventually store the list of all bank 
accounts in the bank.
Add an initialize instance method that sets @balance to zero. You should also add an attr_accessor for balance to your class.
...
Add a class method called create that calls BankAccount.new and adds the new object to @@accounts so that we can find it again 
in the future. This method should return the new account object.
This needs to be a class method because at the time we run it there 
is no single, specific account object that we are working on.
...
class BankAccount
    ...
    @@interest_rate = 0 #what is the interest rate?
    @@accounts = []
    ...
    def total_funds.self
        @@accounts.each do |account|
            sum_of_all_accounts += account
            return sum_of_all_accounts
        end

        def interest_time.self
            @@accounts.map! do |account|
                #increase the amount of the account according to the interest rate
            end
        end
end
